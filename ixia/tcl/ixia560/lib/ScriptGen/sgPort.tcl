...
proc scriptGen::getPortScript { chassis card port } \
{
	...
	if {[card isValidFeature $chassis $card $::portFeature10GEAggregate] &&	\
...
# Check for more parms, so far I know about these
		...
		partiallyGenerateCommand port $portParmList no
		sgPuts {port set $chassis $card $port}
	} elseif {[port isActiveFeature $chassis $card $port $::portFeature1GEAggregate] ||\
		...
		return $retCode
	} elseif {[port isActiveFeature $chassis $card $port $::portFeature10GEAggregate]} {
	...
	# This is for Yukon FC additional port information (Flow Control / Timeout Values / Speeds)
		...
		if { [port isValidFeature $chassis $card $port $::portFeatureStreamExtractor]} {
			...
			if {[lsearch [package names] IxTclProtocol] != -1} {
				set statParamList [concat $statParamList [list enableBgpStats enableIcmpStats enableOspfStats enableIsisStats \
										   ...
										   enableBfdStats enableCfmStats enableLacpStats enableOamStats]]
			}
		}  
    ...
    if {[port isActiveFeature $chassis $card $port $::portFeatureSfpPlus]} {
