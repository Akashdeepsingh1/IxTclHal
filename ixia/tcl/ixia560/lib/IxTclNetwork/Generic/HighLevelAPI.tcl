...
proc ::ixTclNet::StartTraffic {{trafficItemList {}}} {
        ...
        ixNet commit    
        set err ""
        catch { ixNet exec startStatelessTraffic [list [::ixNet getRoot]/traffic ] } err
        if { [string first "no matching exec found" $err ] == "0" } {
            # 5.30 method
	    puts "Use 5.30 to start traffic"
...
proc ::ixTclNet::StopTraffic {} {
        set err ""
        catch { ixNet exec stopStatelessTraffic [list [::ixNet getRoot]/traffic ] } err
        if { [string first "no matching exec found" $err ] == "0" } {
            # 5.30 method
            puts "Use 5.30 to stop traffic"
