...
set ixTclHal::noArgList    { version session chassisChain chassis card qos streamRegion streamQueue streamQueueList \
							 ...
							 txLane pcsLaneStatistics pcsLaneError}
...
set ixTclHal::pointerList   { port protocol arpAddressTableEntry ipAddressTableItem igmpAddressTableItem \
							  ...
							  fcoeDiscoveredInfo fcoePlogi fcoeNameServer ptpProperties ptpDiscoveredInfo\
							  ...
							  dcbxPriorityGroupFeature dcbxPfcFeature dcbxFcoeFeature dcbxLogicalLinkFeature \
                              dcbxCustomFeature dcbxControlTlv}		
																					
## these are basically all the protocols that require a portPtr parameter for instantiation
set ixTclHal::protocolList { ip udp tcp ipx icmp arp dhcp gre macSecTag fcoe fibreChannel \
...
set ixProtocolList {ip udp ptp tcp ipx igmp icmp arp vlan dhcp mpls mplsLabel qos rip ripRoute isl frameRelay ipV6 fcoe }
